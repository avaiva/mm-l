...
authRouter.post('/signup', async (req, res, next) => {
  ...
  try {
    ...
    if (findUser) {
      res.status(401).json('This email is already registered, please go to the login page ');
    }
...
authRouter.post('/login', async (req, res, next) => {
  ...
  try {
    ...
    if (!comparePasswords) {
      res.status(401).json('Wrong password, please make sure you entered correct password');
      return;
    }
